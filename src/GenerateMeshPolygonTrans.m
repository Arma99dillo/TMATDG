function [p_ord,t_ord,I,B] = GenerateMeshPolygonTrans(param)

% Generate mesh with a transmission polygonal obstacle
...
p = mesh.Nodes'; % vertices
t = mesh.Elements'; % indices of the vertices
...
tol = 1e-8; i_int = 1; i_bound = n_nodes;
for i = 1:n_nodes
    % for every node, I check if it is a boundary or internal node
    if abs(p(i,1).^2+p(i,2).^2-R^2) < tol % they are boundary nodes
        old2new(i)=i_bound;
        new2old(i_bound)=i;
        i_bound = i_bound -1;
    else
