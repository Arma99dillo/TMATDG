function A = MatrixDtNTDGTrans(self)

% Build the Dtn-TDG matrix in the transmission case

% get parameters
mesh=self.mesh; param=self.param;
t=mesh.t; p=mesh.p; B=mesh.B; K=param.K; LI=mesh.LI; LDtN=mesh.LDtN; LTrans=mesh.LTrans;
alpha=param.alpha; beta=param.beta; delta=param.delta; epsilon=param.epsilon;
E=mesh.E; M=param.M; R=param.R; nd=param.nd; d=param.d;
...
% first compute DtN expansion coefficients (beta_n)
k = K*sqrt(epsilon(1)); v=1; beta_n=zeros(2*M+1,1); 
for n=-M:M
    beta_n(v)=(n/(k*R)-besselh(n+1,k*R)/besselh(n,k*R));
    v=v+1;
end
% Compute Fourier Coefficients of basis function
FC=FourierCoeff(p,LDtN,k,d,nd,R,M,q,nodes,w);
for L1=1:size(LDtN,1) %global terms: consider all edges
    for L2=1:size(LDtN,1)
        % adjiacent triangles and endpoints
        t1=LDtN(L1,3); t2=LDtN(L2,3); 
        l1=[LDtN(L1,1:2)]; l2=[LDtN(L2,1:2)];
        ...
        for l=1:nd 
            for j=1:nd
                % compute integrals
                [int1,int2,int3]=Dir_to_Neum(p,L1,L2,d,nd,l,j,k,l1,l2,M,beta_n,FC,delta,R,q,nodes,w);
                A_aux(j,l) = A_aux(j,l) - delta*int2 - int1 - (delta/(1i*k))*int3;
            end
        end
        A((t2-1)*nd+1:t2*nd,(t1-1)*nd+1:t1*nd)=A((t2-1)*nd+1:t2*nd,(t1-1)*nd+1:t1*nd)+A_aux;  
    end
...
% Cycle on circular boundary, local terms
for L=1:size(LDtN,1)
    ...
    for l = 1:nd
        for j= 1:nd
            dl = d(:,l); dj = d(:,j); diff=dl-dj;
            f = @(theta) (dj(1)*cos(theta)+dj(2)*sin(theta))*(delta*dl(1)*cos(theta)+delta*dl(2)*sin(theta)+1)*exp(1i*k*(R*cos(theta)*diff(1)+R*sin(theta)*diff(2)));
            A_aux(j,l) = A_aux(j,l) - 1i*k*R*gaussint(f,theta1,theta2,q,nodes,w);
        end
    end
